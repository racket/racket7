...
;; ----------------------------------------
;; Lookahead, lookbehind, and cut

(define (LOOKAHEAD match? sub-m next-m)
  (lambda (s pos start end state stack fail-k)
    (define pos2 (sub-m s pos start end state stack (lambda () #f)))
      ...
      (cond
       [(lb-pos . < . lb-min-pos)
        (if match?
            ...
            (next-m s pos start end state stack fail-k))]
       [else
        (define pos2 (sub-m s lb-pos start end state stack (lambda () #f)))
        (if (eq? match? (and pos2 #t))
            (next-m s pos start end state stack fail-k)
            (loop (sub1 lb-pos)))]))))
