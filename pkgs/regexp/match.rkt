...
(define (interp m s pos start end state)
  (m s pos start end end state null (lambda () #f)))

(define done-m (lambda (s pos start end eos state stack fail-k)
                 pos))
(define continue-m (lambda (s pos start end eos state stack fail-k)
                     ((car stack) pos fail-k)))
(define limit-m (lambda (s pos start end eos state stack fail-k)
                  (= pos end)))
...
(define-syntax-rule (define-plain+simple+tail (general-matcher simple-matcher tail-matcher arg ... next-m)
                      (lambda (s pos start end eos)
                        tst
                        next-pos))
  (begin
    ;; General mode for `next-m` that may backtrack
    (define (general-matcher arg ... next-m)
      (lambda (s pos start end eos state stack fail-k)
        (if tst
            (next-m s next-pos start end eos state stack fail-k)
            (fail-k))))
            ...
            (let loop ([pos2 pos] [n 0])
              (cond
               [(or (and end ((+ pos2 size) . > . end))
                    (not (lazy-bytes-before-end? s (+ pos2 (sub1 size)) end))
                    (not ls-tst))
                (values pos2 n)]
               [else
    ...
    (if (bytes? s)
        (and ((+ pos len) . <= . end)
             (for/and ([c1 (in-bytes bstr 0 len)]
               ...
               (= c1 c2)))
        (and (lazy-bytes-before-end? s (sub1 (+ pos len)) end)
             (for/and ([c1 (in-bytes bstr 0 len)]
                       [i (in-naturals pos)])
...
(define-syntax-rule (define-zero-width (op-matcher arg ... next-m)
  ...
  (define (op-matcher arg ... next-m)
    (lambda (s pos start end eos state stack fail-k)
      (if tst
          (next-m s pos start end eos state stack fail-k)
          (fail-k)))))
...
(define-zero-width (end-matcher next-m)
  (lambda (s pos start end eos)
    (if (bytes? s)
        (= pos eos)
        (not (lazy-bytes-before-end? s pos eos)))))
...
(define-zero-width (line-end-matcher next-m)
  (lambda (s pos start end eos)
    (if (bytes? s)
        ...
        (or (not (lazy-bytes-before-end? s pos eos))
            (= (char->integer #\newline)
               (lazy-bytes-ref s pos))))))

(define-zero-width (word-boundary-matcher next-m)
  (lambda (s pos start end eos)
    (word-boundary? s pos start end eos)))

(define-zero-width (not-word-boundary-matcher next-m)
  (lambda (s pos start end eos)
    (not (word-boundary? s pos start end eos))))

(define (word-boundary? s pos start end eos)
  (not (eq? (or (= pos start)
...
(define (alts-matcher m1 m2)
  (lambda (s pos start end eos state stack fail-k)
    (m1 s pos start end eos state stack
        (lambda ()
          (m2 s pos start end eos state stack fail-k)))))
      ...
      (cond
       [(n . < . min)
        (define new-stack (cons (lambda (pos fail-k)
                                ...
                                stack))
        (r-m s pos start end eos state new-stack fail-k)]
       [(and max (= n max)) (next-m s pos start end eos state stack fail-k)]
       [else
        (define new-stack (cons (lambda (pos fail-k)
                                  (rloop pos (add1 n) fail-k))
                                stack))
        (r-m s pos start end eos state new-stack
             (lambda () (next-m s pos start end eos state stack fail-k)))]))))
...
(define (repeat-simple-matcher r-m min max next-m)
  ...
  (lambda (s pos start end eos state stack fail-k)
    (let rloop ([pos pos] [n 0] [back-amt 0])
      (define pos2
        (if (or (not max) (n . < . max))
            (r-m s pos start end eos state r-stack (lambda () #f))
            #f))
      (if pos2
          ...
          (let bloop ([pos pos] [n n])
            (cond
             [(n . < . min) (fail-k)]
             [else (next-m s pos start end eos state stack
      ...
      (if pos2
          ...
          (let bloop ([pos pos] [n n])
            (cond
             [(n . < . min) (fail-k)]
             [else
              (define pos2 (next-m s pos start end eos state stack (lambda () #f)))
              (or pos2
...
(define (repeat-simple-many-matcher r-m* back-amt min max next-m)
  ...
  (lambda (s pos start end eos state stack fail-k)
    (define-values (pos2 n) (r-m* s pos start end eos))
    ...
    (let rloop ([pos pos] [n 0] [min min] [fail-k fail-k])
      (cond
       ...
       [(and max (= n max))
        (next-m s pos start end eos state stack fail-k)]
       [else
        (next-m s pos start end eos state stack
...
(define (lazy-repeat-simple-matcher r-m min max next-m)
  ...
  (lambda (s pos start end eos state stack fail-k)
    (let rloop ([pos pos] [n 0] [min min] [fail-k fail-k])
      (cond
       [(n . < . min)
        (define pos2 (r-m s pos start end eos state stack (lambda () #f)))
        (if pos2
...
(define (group-push-matcher n next-m)
  (lambda (s pos start end eos state stack fail-k)
    (define new-stack (cons (cons pos (and state (vector-ref state n)))
                            stack))
    (next-m s pos start end eos state new-stack fail-k)))
...
(define (group-set-matcher n next-m)
  (lambda (s pos start end eos state stack fail-k)
    ...
    (next-m s pos start end eos state (cdr stack)
            (lambda ()
          ...
          (if (bytes? s)
              ...
              (and (lazy-bytes-before-end? s (sub1 (+ pos len)) end)
                   (for/and ([j (in-range (car p) (cdr p))]
                     ...
                     (chyte=? c1 c2)))))
        (if matches?
            (next-m s (+ pos len) start end eos state stack fail-k)
            (fail-k))]))))
...
(define (lookahead-matcher match? sub-m n-start num-n next-m)
  (lambda (s pos start end eos state stack fail-k)
    ...
    (define pos2 (sub-m s pos start end eos state null (lambda () #f)))
    (cond
     [match?
      (if pos2
          (next-m s pos start end eos state stack
  ...
  (lambda (s pos start end eos state stack fail-k)
    ...
    (let loop ([lb-pos (- pos lb-min)])
      (cond
       ...
       [else
        (define old-state (save-groups state n-start num-n))
        (define pos2 (sub-m s lb-pos start pos eos state null (lambda () #f)))
        (cond
         [match?
          (if pos2
              (next-m s pos start end eos state stack
...
(define (conditional/reference-matcher n m1 m2)
  (lambda (s pos start end eos state stack fail-k)
    (if (vector-ref state n)
        (m1 s pos start end eos state stack fail-k)
        (m2 s pos start end eos state stack fail-k))))
...
(define (conditional/look-matcher tst-m m1 m2 n-start num-n)
  (lambda (s pos start end eos state stack fail-k)
    ...
    (if (tst-m s pos start end eos state null (lambda () #f))
        (m1 s pos start end eos state stack branch-fail-k)
        (m2 s pos start end eos state stack branch-fail-k))))
...
(define (cut-matcher sub-m n-start num-n next-m)
  (lambda (s pos start end eos state stack fail-k)
    ...
    (define pos2 (sub-m s pos start end eos state null (lambda () #f)))
    (if pos2
        (next-m s pos2 start end eos state stack
                (make-restore-groups fail-k state old-state n-start num-n))
        (fail-k))))
                   ...
                   (let ([c-cat (char-general-category c)])
                     (if (list? cats)
                         (for/or ([cat (in-list cats)])
                           (eq? cat c-cat))
                         (eq? cats c-cat))))
              (next-m s (add1 pos) start end eos state stack fail-k)
              (fail-k))]
         [(eq? c 'fail)
          (fail-k)]
         [else
